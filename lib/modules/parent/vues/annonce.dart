...
class AnnonceModele {
  ...
  final List<String> utilisateursConcernees; // note : pluriel correct
...
class ListeAnnoncesParentPage extends StatefulWidget {
  final String utilisateurId;
  final String etablissementId;

  const ListeAnnoncesParentPage({
    Key? key,
  ...
  @override
  State<ListeAnnoncesParentPage> createState() => _ListeAnnoncesParentPageState();
}

class _ListeAnnoncesParentPageState extends State<ListeAnnoncesParentPage> {
  bool isLoading = true;
  String? error;
  ...
  void initState() {
    super.initState();
    print('[initState] Chargement des annonces pour parent utilisateurId=${widget.utilisateurId}');
    _chargerAnnonces();
  }

  Future<void> _chargerAnnonces() async {
    try {
      print('[chargerAnnonces] Début du chargement...');
      setState(() {
        isLoading = true;
        error = null;
      });
      ...
      print('[chargerAnnonces] Nombre d\'annonces récupérées : ${querySnap.docs.length}');

      annonces = querySnap.docs
          .map((doc) {
            print('[chargerAnnonces] Annonce chargée: id=${doc.id}, titre=${doc['titre']}');
      ...
      print('[chargerAnnonces] Fin du chargement.');
      ...
      print('[chargerAnnonces] FirebaseException: ${e.message}');
      ...
      if (e.message != null && e.message!.contains('https://console.firebase.google.com')) {
        ...
        if (match != null) {
          print('[chargerAnnonces] Lien index Firestore recommandé : ${match.group(0)}');
        }
      }
    } catch (e) {
      setState(() {
      ...
      print('[chargerAnnonces] Erreur inattendue : $e');
    }
  }
  ...
  Future<void> _marquerCommeLue(String annonceId) async {
    ...
    print('[marquerCommeLue] Annonce à marquer comme lue: $annonceId');
    try {
      await docRef.update({
        'luePar': FieldValue.arrayUnion([widget.utilisateurId]),
      });
      print('[marquerCommeLue] Mise à jour réussie pour annonce $annonceId');
      ...
      print('[marquerCommeLue] Erreur lors de la mise à jour : $e');
      if (mounted) {
